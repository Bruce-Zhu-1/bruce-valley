...
function DiaryCommentBoard({ diaryId }) {
  ...
  const loadLikedComments = () => {
    const stored = localStorage.getItem(`liked_diary_comments_${diaryId}`)
    if (stored) {
  ...
  const saveLikedComments = (newList) => {
    ...
    localStorage.setItem(`liked_diary_comments_${diaryId}`, JSON.stringify(newList))
  }
...
export default DiaryCommentBoard
